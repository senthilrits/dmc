...
| `bootcampdocs/` | The **DM Extensibility Bootcamp** — the guided-tutorial layer that ties the `dm-*` samples together. Lecture units (`index.md` Unit 1 … `MachineExtensions.md` Unit 8, plus `BTPExtensionSuite.md`, `DMCUseCases.md`, `ParticipantGuide.md` for BTP/Kyma trial setup, `FAQ.md`) and hands-on exercises (`Exercise1.1`–`Exercise5`), all built around one end-to-end lifter-assembly scenario (`BootcampUseCase.md`: plant EBC200/300/400, WC-LIFT, material LIFTER-ASSY). **Each exercise points at a specific sample folder** — e.g. Ex 1.2 → `dm-inapp-service-extensions/sample-service-extension`, Ex 2.1 → `api-mssql-nodejs`, Ex 4.1 → `dm-sidebyside-extension/SideBySide_*`, Ex 4.2 → `dm-podplugin-extensions/PodPlugin_*`, Ex 4.3 → `dm-integration-extensions/Z_PRODORD_*`. So read it to understand *why* a sample exists and how it's used. Caveats: `KPIExtensions.md` (Unit 10), `Summary.md` (Unit 11), `Reference.md` are unfinished stubs (Unit 9 is missing); `README.md` is a hand-maintained link index pointing at the upstream repo, not relative paths; no `mkdocs.yml` is committed (the vendored `theme/` is unused) so there's no build target; and the docs contain throwaway bootcamp secrets (a Teams webhook URL, `SA_PASSWORD=DMC_Bootcamp123`, `.dmcbak` password `BootCamp2023:)`) — do not treat as live credentials | Documentation only |
